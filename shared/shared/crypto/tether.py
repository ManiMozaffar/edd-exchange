...
class TetherCrypto(AbstractCrypto):
    ...
    def get_price_from_units(self, currency: Type[Currency] = USD) -> Currency:
        ...
        usd_obj = USD(self.amount * self.get_rate_from_usd())
        return usd_obj.to(currency)

    def get_units_from_price(self, currency: Currency) -> "TetherCrypto":
        usd_obj = USD(currency)
        self.amount = usd_obj.amount / self.get_rate_from_usd()
        return self
